...
data %>% 
  ...
  ggplot(aes(x=str_wrap(name, width = 20),y=Percentage))+
...
monthsSR<-round(mean(data_SRAcademics$`On average, how many months did it take you to perform a systematic review (from protocol writing to a first draft of a publication)?  []`,na.rm = T))

```

Researchers have been involved in a median of `r medianSRConducted`, with a range of `r minSRConducted` - `r maxSRConducted`. It took an avarage of `r monthsSR` months to complete a SR (from protocol writing to a first draft of a publication).
...
meanAge<-round(mean(data_demographics$`Your age:  []`),1)
...
```
`r nFemales` (`r round((nFemales/nrow(data_demographics))*100)`%) of the participants were females. Average age was `r meanAge` (SD: `r sdAge`, range: `r minAge` - `r maxAge`)
